...
var hooks = getAllFiles("./src/hooks").map(function (x) {
  return x.split("src/")[1];
});
var components = getAllFiles("./src/components")
  .filter(function (x) {
    return !ignoreFolders.some(function (igf) {
      return x.includes(igf);
    });
  }) // Menggunakan `includes` agar lebih fleksibel
  .map(function (x) {
    return x.split("src/")[1];
  });
exports.ui = [
  {
    ...
    files: __spreadArray(
      __spreadArray(__spreadArray([], hooks, true), components, true),
      ["lib/menu-list.ts"],
      false,
    ),
  },
