...
const IGNORE_RE = /^\/(_next\/static|_next\/image|favicon\.ico|api\/health|api\/proxy-image)/;
const STATIC_EXT_RE =
...
app
  ...
  .then(() => {
    ...
    server.listen(port, hostname, () => {
      logger.info(
        `FS-Public production server ready on ${hostname}:${port} (${process.env.NODE_ENV ?? "production"})`,
      );
    });
  })
  .catch((err) => {
    logger.error({ err }, "FS-Public production server failed to start");
    process.exit(1);
  });
