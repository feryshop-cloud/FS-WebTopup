...
function CredentialValue({
  ...
  return (
    <div className="border-primary/10 bg-muted/20 space-y-2 rounded-2xl border p-4">
...
export default function AccountSettingsApiPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const nextSecret =
        typeof json?.data?.plain_secret_key === "string" ? json.data.plain_secret_key : "";
  ...
  if (!isAuthed) {
    return (
      <ContentLayout title="Pengaturan API">
        <div className="border-primary/20 bg-card rounded-3xl border border-dashed p-10 text-center shadow-sm">
          <Badge className="border-primary/15 bg-primary/10 text-primary hover:bg-primary/10 rounded-full border px-3 py-1">
            Pengaturan API
          </Badge>
          <h1 className="mt-5 text-2xl font-bold tracking-tight">
            Masuk untuk mengelola credential API partner
          </h1>
          <p className="text-muted-foreground mx-auto mt-3 max-w-xl text-sm leading-7">
            Halaman ini dipakai untuk membuat API key per user, mengatur secret key custom, dan
            mengaktifkan Client API v1.
          </p>
          <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
            <Button onClick={() => signIn()} className="rounded-full px-6">
              Login Sekarang
            </Button>
            <Button asChild variant="outline" className="border-primary/10 rounded-full px-6">
  ...
  return (
    <ContentLayout title="Pengaturan API">
      <div className="mx-auto w-full max-w-6xl space-y-6">
        <section className="border-primary/10 bg-card relative overflow-hidden rounded-3xl border p-6 shadow-sm sm:p-8">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <h1 className="text-3xl font-bold tracking-tight">
                  Kelola API key, secret key, dan status akses partner langsung dari akun kamu.
                </h1>
                <p className="text-muted-foreground max-w-2xl text-sm leading-7">
                  Source code terbaru sudah menyediakan credential API per user. Dari sini kamu bisa
                  generate credential, rotate key, membuat secret custom, dan mengaktifkan atau
                  menonaktifkan akses Client API v1.
                </p>
              </div>
            ...
            <Card className="border-primary/10 bg-background/80 rounded-3xl shadow-none lg:min-w-[320px]">
              <CardContent className="space-y-4 p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-semibold">
                      {meLoading ? "Memuat..." : me?.data?.name || "User"}
                    </div>
                    <div className="text-muted-foreground text-xs">{displayIdentity}</div>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge
                    variant={credential?.is_active ? "default" : "outline"}
                    className="rounded-full px-3 py-1"
                  >
                    {credentialStatusText}
                  </Badge>
                </div>
                <div className="border-primary/10 bg-muted/30 text-muted-foreground rounded-2xl border p-4 text-sm leading-7">
                  Gunakan credential ini hanya di backend server kamu. Jangan expose secret key atau
                  algoritma signature ke browser publik dan mobile client.
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <Card className="border-primary/10 rounded-3xl shadow-sm">
            <CardHeader>
            ...
            <CardContent className="text-muted-foreground space-y-3 text-sm leading-7">
              <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                Status:{" "}
                <span className="text-foreground font-semibold">{credentialStatusText}</span>
              </div>
              <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                Rotated at:{" "}
          ...
          <Card className="border-primary/10 rounded-3xl shadow-sm">
            ...
            <CardContent className="text-muted-foreground space-y-3 text-sm leading-7">
              ...
              <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                Last used IP:{" "}
                <span className="text-foreground font-semibold">
                  {credential?.last_used_ip || "Belum ada"}
                </span>
              </div>
              <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                Scope:{" "}
                <span className="text-foreground font-semibold">Client API v1 signed request</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-primary/10 rounded-3xl shadow-sm">
            ...
            <CardContent className="text-muted-foreground space-y-3 text-sm leading-7">
              <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                Simpan API key dan secret key hanya di environment backend atau vault.
              </div>
              <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                Setiap create order partner wajib memakai Idempotency-Key unik.
              </div>
              <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                Jika credential bocor, regenerate API key dan update secret secepat mungkin.
              </div>
            </CardContent>
          </Card>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <Card className="border-primary/10 rounded-3xl shadow-sm">
            <CardHeader>
              ...
              <CardDescription>
                Generate, tampilkan, copy, rotate, dan update credential sesuai implementasi backend
                terbaru.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              {credentialLoading ? (
                ...
                <div className="border-primary/20 bg-muted/20 space-y-4 rounded-2xl border border-dashed p-6">
                  <div className="text-lg font-semibold">Credential belum dibuat</div>
                  <p className="text-muted-foreground text-sm leading-7">
                    Generate credential untuk mengaktifkan Client API v1. Secret asli hanya akan
                    tampil sekali setelah proses generate berhasil.
                  </p>
                  <Button
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-primary/10 bg-muted/20 space-y-2 rounded-2xl border p-4">
                    ...
                    <Input
                      readOnly
                      value={credential.secret_key_masked}
                      className="rounded-xl font-mono text-xs"
                    />
                    <p className="text-muted-foreground text-xs leading-6">
                      Backend hanya mengembalikan versi masked setelah credential tersimpan. Simpan
                      plain secret di tempat aman saat pertama kali generate atau saat kamu membuat
                      secret custom baru.
                    </p>
                  </div>

                  {plainSecret ? (
                    <div className="border-primary/10 bg-background/70 space-y-3 rounded-2xl border p-4">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <div className="font-semibold">Plain secret terbaru</div>
                          <p className="text-muted-foreground text-xs leading-6">
                            Tampilkan hanya di lingkungan aman. Setelah halaman ditutup, nilai ini
                            tidak akan muncul lagi dari backend.
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            onClick={() => setShowPlainSecret((state) => !state)}
                          >
                            {showPlainSecret ? (
                          ...
                          <Button
                            ...
                            size="sm"
                            className="rounded-full"
                            onClick={() => copyText(plainSecret, "Secret key berhasil disalin.")}
                          >
                            Salin
                          </Button>
                        </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="border-primary/10 bg-background/70 rounded-2xl border p-4">
                      <div className="font-semibold">Rotate API key</div>
                      <p className="text-muted-foreground mt-2 text-sm leading-7">
                        Mengganti api_key tanpa mengubah secret key saat ini.
                      </p>
                      <Button
                    ...
                    <div className="border-primary/10 bg-background/70 rounded-2xl border p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-muted-foreground mt-2 text-sm leading-7">
                            Nonaktifkan credential saat tidak dipakai atau saat ada indikasi
                            kebocoran.
                          </p>
                        </div>
                        <Switch
                      ...
                      <div className="text-muted-foreground mt-4 text-xs">
                        {togglingStatus
                          ...
                          : credential.is_active
                            ? "Credential aktif dan bisa dipakai signed request."
                            : "Credential nonaktif dan akan ditolak middleware."}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <Card className="border-primary/10 rounded-3xl shadow-sm">
            <CardHeader>
              ...
              <CardDescription>
                Buat secret baru agar pola penamaan mengikuti standar tim kamu.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                />
                <p className="text-muted-foreground text-xs leading-6">
                  Minimal 24 karakter, maksimal 120 karakter, hanya huruf, angka, titik, garis
                  bawah, dan strip, serta wajib mengandung huruf dan angka.
                </p>
              </div>
              <Button
                onClick={handleSaveSecret}
                disabled={savingSecret || !credential}
                className="rounded-full px-6"
              >
                {savingSecret ? (
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ...
              <div className="text-muted-foreground space-y-3 text-sm leading-7">
                <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                  Gunakan secret yang berbeda antara production dan sandbox.
                </div>
                <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                  Setelah secret diubah, semua signer di backend partner wajib ikut diperbarui
                  sebelum request baru dikirim.
                </div>
                <div className="border-primary/10 bg-muted/20 rounded-2xl border p-4">
                  Jika kamu baru generate credential dan belum pernah menyimpan secret, salin plain
                  secret terbaru sekarang juga.
                </div>
              </div>
            </CardContent>
          </Card>
