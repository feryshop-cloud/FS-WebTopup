...
export async function GET(_req: Request, context: any) {
  try {
    ...
    if (!liveGame) {
      return NextResponse.json(
        { success: false, message: "Game tidak ditemukan di database" },
        { status: 404 },
      );
    }
    ...
    const requiredInputs =
      Array.isArray(instructionsObj.required_inputs) && instructionsObj.required_inputs.length > 0
        ? instructionsObj.required_inputs
        : ["id", "server"];

    const inputFields =
      Array.isArray(instructionsObj.input_fields) && instructionsObj.input_fields.length > 0
