...
export function InvoicePaymentInstructions({ order }: InvoicePaymentInstructionsProps) {
  const fallback =
    !order.payment_instructions?.length && isSmpQrisOrder(order) ? buildSmpQrisInstructions() : [];

  const instructions = (
    order.payment_instructions?.length ? order.payment_instructions : fallback
  ...
  return (
    <div className="col-span-12">
      <div className="flex flex-col gap-2">
        <div className="border-border/25 bg-secondary/50 mt-2 rounded-lg border p-4 text-sm">
            ...
            <div key={index} className="mb-4">
              ...
              <ol className="list-decimal pl-5">
                {instruction.steps.map((step, i) => (
                  <li key={i} className="mt-1" dangerouslySetInnerHTML={{ __html: step }} />
                ))}
              </ol>
            </div>
          ))}
