    ...
    <div className="mx-auto w-full max-w-3xl space-y-6">
      <div className="bg-background rounded-2xl border p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="space-y-1">
            <div className="text-muted-foreground text-sm">Nomor Invoice</div>
            ...
            <div className="text-muted-foreground text-xs">
              Metode: <span className="text-foreground font-medium">{order.payment_name}</span>
            </div>
          </div>

          <div className="space-y-1 text-left md:text-right">
            <div className="text-muted-foreground text-sm">Total Pembayaran</div>
            <div className="text-xl font-bold">
              Rp {Number(order.total_price || 0).toLocaleString("id-ID")}
            </div>
            <div className="text-muted-foreground text-xs">{priceLine}</div>
          </div>
        </div>
      </div>
      ...
      <div className={`rounded-2xl border p-5 ${getBackgroundBuyStatusColor()}`}>
        <div className="space-y-2">
          ...
          <div className="text-muted-foreground text-sm">{getBuyStatusMessage()}</div>
        </div>
      </div>
        ...
        <div className="bg-background rounded-2xl border p-5">
          ...
          {(isRetailMethod || isVaMethod) && (
            <div className="mt-4 flex items-center gap-2">
              <div className="bg-muted flex-1 break-all rounded-lg border px-3 py-2 text-sm">
                {String(order.payment_code_display || order.payment_code || "-")}
              </div>
              <Button
                variant="outline"
            ...
            <div className="mt-5 space-y-3">
              {isQrispy && qrImageSrc && (
                <div className="flex flex-col items-center gap-3">
                  <div className="rounded-2 relative h-[260px] w-[260px] overflow-hidden border bg-white">
                    <Image
                      src={qrImageSrc}
                      alt="QRIS"
                      fill
              ...
              {(isQrisStringMethod ||
                Boolean(explicitQrString) ||
                (isQrisImageMethod && !isUrlPayCode && Boolean(qrString))) &&
                qrString && (
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="bg-muted w-full break-all rounded border p-3 text-xs">
                      {qrString}
                    </div>
