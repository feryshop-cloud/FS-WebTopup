...
export function InvoicePaymentDetails({ order }: InvoicePaymentDetailsProps) {
  ...
  const unitPriceBeforePromo = money(
    pricing?.unit_price_before_promo ?? order.discount_price ?? order.price ?? 0,
  );
  const unitPriceAfterPromo = money(pricing?.unit_price_after_promo ?? order.price ?? 0);
  const subtotalBeforePromo = money(
    pricing?.subtotal_before_promo ?? unitPriceBeforePromo * quantity,
  );
  ...
  const hasPromo = Boolean(
    (pricing?.has_promo ?? false) || (promoCode !== "" && promoDiscountTotal > 0),
  );
  const showUniqueCode = isSmp && uniqueCode > 0;

  return (
    <div className="border-border bg-muted/50 space-y-4 rounded-xl border p-4 text-sm">
      <div className="space-y-3">
        ...
        <div className="flex justify-between gap-4">
          <span className="text-foreground font-medium">Harga Satuan</span>
          <span className="text-muted-foreground">
            Rp {unitPriceAfterPromo.toLocaleString("id-ID")}
          </span>
        </div>
        ...
        {hasPromo ? (
          <>
            <div className="flex justify-between gap-4">
              <span className="text-foreground font-medium">Subtotal Sebelum Promo</span>
              <span className="text-muted-foreground">
                Rp {subtotalBeforePromo.toLocaleString("id-ID")}
              </span>
            </div>
            ...
            <div className="flex justify-between gap-4">
              ...
              <span className="text-muted-foreground">
                -Rp {promoDiscountTotal.toLocaleString("id-ID")}
              </span>
            </div>

            <div className="flex justify-between gap-4">
              <span className="text-foreground font-medium">Subtotal Setelah Promo</span>
              <span className="text-muted-foreground">
                Rp {subtotalAfterPromo.toLocaleString("id-ID")}
              </span>
            </div>
          </>
        ) : (
        ...
        {showUniqueCode ? (
          <div className="flex justify-between gap-4">
            ...
            <span className="text-my-color font-semibold">
